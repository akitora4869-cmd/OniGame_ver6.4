...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void startIkimonoSystem(Location center){
        ...
        for(int i=0;i<count;i++){
            ...
            for(int tries=0;tries<24;tries++){
                ...
                Location c=new Location(center.getWorld(),x+.5,y,z+.5);
                if(c.getBlock().isPassable()&&c.clone().add(0,1,0).getBlock().isPassable()){loc=c;break;}
            }
    ...
    private Location findIkimonoCommandSpawn(Location base){
        ...
        for(int tries=0;tries<16;tries++){double ang=r.nextDouble()*Math.PI*2,dist=3+r.nextDouble()*5;int x=base.getBlockX()+(int)Math.round(Math.cos(ang)*dist),z=base.getBlockZ()+(int)Math.round(Math.sin(ang)*dist),y=w.getHighestBlockYAt(x,z)+1;Location c=new Location(w,x+.5,y,z+.5);if(c.getBlock().isPassable()&&c.clone().add(0,1,0).getBlock().isPassable())return c;}
        return base.clone().add(2,0,2);
    }
    private void clearIkimonoEntitiesOnly(){for(UUID id:new HashSet<>(ikimonoIds)){clearIkimonoLight(id);Entity e=Bukkit.getEntity(id);if(e!=null)e.remove();}ikimonoIds.clear();ikimonoLastAttacker.clear();}
    ...
    private void clearIkimonoSystem(){if(ikimonoTicker!=null){ikimonoTicker.cancel();ikimonoTicker=null;}for(UUID id:new HashSet<>(ikimonoIds)){clearIkimonoLight(id);Entity e=Bukkit.getEntity(id);if(e!=null)e.remove();}for(UUID id:new HashSet<>(ikimonoLightLocations.keySet()))clearIkimonoLight(id);ikimonoIds.clear();ikimonoLastAttacker.clear();ikimonoBotBuffs.clear();ikimonoFactionBuffUntil.clear();}
    private void handleIkimonoDeath(EntityDeathEvent e){
        e.getDrops().clear();e.setDroppedExp(0);
        String type=randomIkimonoBuffType();UUID killer=ikimonoLastAttacker.remove(e.getEntity().getUniqueId());
        if(killer!=null&&isGameBot(killer)){
            ikimonoBotBuffs.computeIfAbsent(killer,k->new ArrayDeque<>()).addLast(type);
            all("&5異獣がBotに討伐された。 &d特殊な加護 &7を確保した……");
        }else{e.getDrops().add(createIkimonoBuff(type));all("&5異獣が討伐された。 &d特殊な加護 &7が落ちた……");}
    ...
    @EventHandler public void onDamage(EntityDamageByEntityEvent e){
        ...
        if(ikimonoIds.contains(e.getEntity().getUniqueId())&&(rawAttacker instanceof Player||isGameBot(rawAttacker.getUniqueId())))ikimonoLastAttacker.put(e.getEntity().getUniqueId(),rawAttacker.getUniqueId());
